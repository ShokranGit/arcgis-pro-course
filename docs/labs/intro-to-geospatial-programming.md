...
<p>Add <code>NUTS_RG_10M_2021_4326_LEVL_1</code> to your ArcGIS Pro project. Go to the Map tab &rarr; Add Data, browse to the extracted folder, and select <code>NUTS_RG_10M_2021_4326_LEVL_1.shp</code>. The layer loads immediately in a default single-colour symbology.</p>
<ol>
<li>In the Contents panel, click the layer name once to select it. Then go to the Feature Layer contextual tab at the top ribbon &rarr; Appearance group &rarr; click Symbology.</li>
<li>In the Symbology pane that opens on the right, click the Primary Symbology dropdown and choose Unique Values. In the Field 1 dropdown select CNTR_CODE. Then click the Add All Values button at the bottom of the pane. ArcGIS Pro assigns a distinct colour to each of the 37 country codes.</li>
<li>Zoom to the layer extent (right-click the layer in Contents &rarr; Zoom to Layer). You will see 37 differently coloured countries, each subdivided into their NUTS Level-1 regions. Hover over a few polygons and read the pop-up to confirm the CNTR_CODE and NAME_LATN fields.</li>
</ol>
<div class="ref-figure">
<img src="../arcpy-nuts1-symbolized.png" alt="NUTS Level-1 regions symbolized by CNTR_CODE in ArcGIS Pro">
<p class="ref-caption">Figure 1 &mdash; NUTS Level-1 regions symbolized by CNTR_CODE. Each colour represents one country; the 125 features include overseas territories.</p>
</div>
...
<ol>
<li>In the Python window, scroll up so the complete output is visible &mdash; shape type, spatial ref, feature count, field list, and the sorted list of country codes. Verify Countries found: 37.</li>
</ol>
<div class="ref-figure">
<img src="../arcpy-inspect-output.png" alt="Python window showing arcpy inspection output for the NUTS Level-1 layer">
<p class="ref-caption">Figure 2 &mdash; Python window showing inspection output: shape type Polygon, spatial reference GCS_WGS_1984, feature count 125, field list, and Countries found: 37.</p>
</div>
...
<p>After running, the <code>Europe_Countries</code> feature class appears as a new layer in the Contents panel. Symbolize it by CNTR_CODE (Unique Values, same steps as Part 1) and zoom to its extent. Each country is now a single polygon &mdash; France, Portugal, and Spain include their overseas territories as multi-part polygons.</p>
<ol>
<li>Right-click Europe_Countries in Contents &rarr; Attribute Table. Confirm the record count in the bottom bar reads exactly 37 features &mdash; one row per country code.</li>
<li>Zoom to the Europe_Countries layer extent. Compare the result to the original NUTS layer: the internal region boundaries are gone; only country outlines remain.</li>
</ol>
<div class="ref-figure">
<img src="../arcpy-europe-countries-dissolve.png" alt="Europe_Countries after Dissolve symbolized by CNTR_CODE">
<p class="ref-caption">Figure 3 &mdash; Europe_Countries after Dissolve: one multi-part polygon per country (37 features total).</p>
</div>
...
<p>After the script finishes, the Python window will show one "Exported: XX" line per country followed by a confirmation that all files were written. Scroll the Python window up to see all the output lines from the beginning of the run.</p>
<ol>
<li>Scroll the Python window to show all the "Exported: XX" confirmation lines. Confirm the count matches the number of country codes found in Part 2 (37).</li>
</ol>
<div class="ref-figure">
<img src="../arcpy-export-loop-output.png" alt="Python window showing the export loop printing one Exported line per country code">
<p class="ref-caption">Figure 4 &mdash; Python window showing the export loop: one "Exported: XX" line per country code, confirming 37 shapefiles written.</p>
</div>
<ol start="2">
<li>Open Windows File Explorer and navigate to your <code>by_country</code> folder. Sort by file name. Count the .shp files &mdash; there should be exactly 37, one per country code (AL_NUTS1.shp, AT_NUTS1.shp ... UK_NUTS1.shp).</li>
</ol>
</div>
</div>
